...
fn bench_agent_creation(c: &mut Criterion) {
    ...
    for n in [1, 10, 100, 1_000] {
        ...
        group.bench_with_input(BenchmarkId::new("steps", n), &steps, |b, steps| {
            b.iter(|| black_box(Agent::new("bench-agent", steps.clone())));
        });
    }
    group.finish();
}
...
fn bench_agent_serde(c: &mut Criterion) {
    ...
    for n in [1, 10, 100] {
        ...
        group.bench_with_input(
            BenchmarkId::new("serialize_steps", n),
            &agent,
            |b, agent| {
        ...
        group.bench_with_input(
            BenchmarkId::new("deserialize_steps", n),
            &json,
            |b, json| {
...
fn bench_context_ops(c: &mut Criterion) {
    ...
    for n in [10, 100, 1_000] {
        ...
        for i in 0..n {
            ctx.set_step_output(
                &StepId::new(format!("s{i}")),
                Value::from(serde_json::json!(i)),
            );
        }
        let last_key = StepId::new(format!("s{}", n - 1));

        group.bench_with_input(
            BenchmarkId::new("get_step_output", n),
            &(ctx, last_key),
            |b, (ctx, key)| {
